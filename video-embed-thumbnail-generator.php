...
function kgvid_video_formats() {
	...
	$video_formats = array(
		"fullres" => array(
			"name" => __("Replace original with H.264"),
			"width" => 4096,
			"height" => 2304,
			"type" => "h264",
			"suffix" => "-fullres.mp4",
...
function kgvid_check_if_capable($capability) {
	...
	foreach ( $wp_roles->roles as $role => $role_info ) {
		if ( array_key_exists($capability, $role_info['capabilities']) && $role_info['capabilities'][$capability] == 1 ) {
			$capable[$role] = "on";
		}
...
function enqueue_kgvid_script() { //loads plugin-related scripts in the admin area
    ...
    wp_enqueue_script( 'video_embed_thumbnail_generator_script', plugins_url('/js/kgvid_video_plugin_admin.js', __FILE__), array('jquery'), $options['version'] );
			...
			foreach ( $id_array as $id ) { //loop through videos
				...
				if ( !empty($id) || !empty($query_atts['caption']) || $content == plugins_url('/images/sample-video-h264.mp4', __FILE__) ) { //generate content below the video
					...
					if ( !empty($query_atts['caption']) || $show_views || $query_atts['downloadlink'] == "true" ) {
						...
						if ( $show_views ) { $code .= '<div class="kgvid-viewcount" id="video_'.$div_suffix.'_viewcount">'.sprintf( _n( '%s view', '%s views', intval(str_replace(',', '', $view_count)) ), $view_count ).'</div>'; }
						if ( !empty($query_atts['caption']) || $query_atts['downloadlink'] == "true" ) {
							...
							if ( $query_atts['downloadlink'] == "true" ) {
								...
								$code .= '<a href="'.$content.'">'.__('Right-click or ctrl-click on this link to download').'</a>';
							}
							$code .= '</div>';
						}
						$code .= '</div>';
				...
				if ( $query_atts['title'] != "false" || $query_atts['embedcode'] != "false" ) { //generate content overlaid on video
					...
					if ( $query_atts['embedcode'] != "false" ) {
						...
						$code .= "\t\t\t\t<div id=\"video_".$div_suffix."_embed\" class=\"kgvid_share\"><span>"._x('Embed:', 'precedes code for embedding video')." </span><input type=\"text\" value=\"".$iframecode."\" onClick=\"this.select();\"></div>\n";
					}
...
function kgvid_generate_encode_checkboxes($movieurl, $post_id, $page) {
	...
	if ( !empty($movieurl) ) {
		...
		$encode_disabled = ' disabled title="'.__('Please enter a valid video URL').'"';
		unset($video_formats['fullres']);
	}

	if ( $options['ffmpeg_exists'] == "notinstalled" ) {
		$ffmpeg_disabled_text = 'disabled="disabled" title="'.sprintf( _x('%1$s not found at %2$s', 'FFMPEG not found at /usr/local/bin'), strtoupper($options['video_app']), $options['app_path']).'"';
	}
	...
	foreach ( $video_formats as $format => $format_stats ) {
			...
			if ( $encodevideo_info[$format.'_exists'] ) { //if the video file exists
				if ( $format_stats['status'] != "encoding" ) { // not currently encoding
					...
					if ( $format_stats['status'] != "canceling" ) {
						...
						&& $format != "fullres" ) {
							$meta[$format] .= '<a id="delete-'.$post_id.'-'.$format.'" class="kgvid_delete-format" onclick="kgvid_delete_video(\''.$movieurl.'\', \''.$post_id.'\', \''.$format.'\', \''.$child_id[$format].'\');" href="javascript:void(0)">'.__('Delete Permanently').'</a>';
						}
					}
		...
		if ( $format_stats['status'] == "encoding" ) {
			$encoding_now = true;
			$disabled[$format] = ' disabled title="'.__('Currently encoding').'"';
		...
		if ( $format_stats['status'] == "Encoding Complete" ) {
			$disabled[$format] = ' disabled title="'.__('Format already exists').'"';
			$checked[$format] = '';
		}
		...
		if ( !current_user_can('encode_videos') ) {
			$disabled[$format] = ' disabled title="'.__('You don\'t have permission to encode videos').'"';
			$something_to_encode = false;
		}
	...
	if ( $something_to_encode == false ) {
		$encode_disabled = ' disabled title="'.__('Nothing to encode').'" style="display:none;"';
	}

	if ( $page == "queue" ) {
		$button_text = _x('Update', 'Verb');
		$checkboxes .= "\n\t\t\t".'<input type="hidden" name="attachments['.$post_id.'][kgflashmediaplayer-url]" value="'.$movieurl.'">';
	}
	else { $button_text = _x('Encode', 'Verb'); }
	...
	if ( $page != "queue" ) {
		$checkboxes .= '<small><em>'.__('Generates additional video formats compatible with most mobile & HTML5-compatible browsers').'</em></small>';
	}
...
function kgvid_generate_queue_table() {
	...
	if ( !empty($video_embed_queue) ) {
		...
		foreach ( $video_embed_queue as $order => $video_entry ) {
			...
			else { $html .= "<td colspan='5'><strong class='kgvid_queue_message'>".__('Other user\'s video')."</strong></td>"; }
			$html .= "</td></tr>\n";
		}
	}
	else { $html = "\t<tr><td colspan='6'><strong class='kgvid_queue_message'>".__('Queue is empty')."</strong></td></tr>\n"; }
...
function kgvid_add_FFMPEG_Queue_Page() {
	...
	if ( $options['ffmpeg_exists'] == "on" ) { //only add the queue page if FFMPEG is installed
		add_submenu_page('tools.php', _x('Video Embed & Thumbnail Generator Encoding Queue', 'Tools page titl'), _x('Video Encode Queue', 'Title in admin sidebar'), 'encode_videos', 'kgvid_video_encoding_queue', 'kgvid_FFMPEG_Queue_Page');
	}
}
...
?>
	<div class="wrap">
		...
		<h2><?php _e('Video Embed & Thumbnail Generator Encoding Queue') ?></h2>
		<p></p>
		...
		<table class="widefat">
			<thead>
				<tr>
				<th><?php _ex('Order', 'noun, column header') ?></th>
				<th><?php _ex('User', 'username, column header') ?></th>
				<th><?php _ex('Thumbnail', 'noun, column header') ?></th>
				<th><?php _ex('File', 'noun, column header') ?></th>
				<th><?php _ex('Formats', 'noun, column header') ?></th>
				<th><?php _ex('Actions', 'noun, column header') ?></th>
			</thead>
			<tfoot>
				<tr>
				<th><?php _ex('Order', 'noun, column header') ?></th>
		...
		<p>
			<input id='clear_completed' type='button' class='button-secondary' value='<?php _e('Clear All Completed') ?>' onclick='kgvid_encode_queue("clear_completed", 0, 0);'>
		</p>
		</form>
	</div>
<?php
...
function kgvid_addFMPOptionsPage() {
		add_options_page( _x('Video Embed & Thumbnail Generator', 'Settings page title'), __('Video Embed & Thumbnail Generator', 'Settings page title in admin sidebar'), 'administrator', __FILE__, 'kgvid_FMPOptionsPage' );
}
...
function kgvid_FMPOptionsPage() {
	...
	<div class="wrap">
		...
		<h2 class="nav-tab-wrapper">
			<a href="#" id="general_tab" class="nav-tab" onclick="kgvid_switch_settings_tab('general');"><?php _ex('General', 'Adjective, tab title') ?></a>
			<a href="#" id="encoding_tab" class="nav-tab" onclick="kgvid_switch_settings_tab('encoding');"><span class='video_app_name'><?php echo strtoupper($video_app); ?></span> <?php _ex('Settings', 'FFMPEG Settings, tab title'); ?></a>
		</h2>
		...
		<?php do_settings_sections(__FILE__); ?>
     	<p class='submit'>
   		   <?php submit_button(__('Save Changes'), 'primary', 'kgvid_submit', false, array( 'onclick' => "jQuery('form :disabled').prop('disabled', false);" ) ); ?>
   		   <?php submit_button(__('Reset Options'), 'secondary', 'video-embed-thumbnail-generator-reset', false); ?>
   		</p>
		</form>

		<div class="kgvid-blue-box">
		<span><?php _e('If you\'re getting some use out of this plugin, please consider donating a few dollars to support its future development.') ?></span>
...
function kgvid_video_embed_options_init() {
	...
	add_settings_section('kgvid_video_embed_playback_settings', __('Default Video Playback Settings'), 'kgvid_plugin_playback_settings_section_callback', __FILE__);
	add_settings_section('kgvid_video_embed_flash_settings', __('The following options will only affect Flash playback'), 'kgvid_plugin_flash_settings_section_callback', __FILE__);
	add_settings_section('kgvid_video_embed_plugin_settings', __('Plugin Settings'), 'kgvid_plugin_settings_section_callback', __FILE__);
	add_settings_section('kgvid_video_embed_encode_settings', __('Video Encoding Settings'), 'kgvid_plugin_settings_section_callback', __FILE__);

	add_settings_field('poster', __('Default thumbnail:'), 'kgvid_poster_callback', __FILE__, 'kgvid_video_embed_playback_settings', array( 'label_for' => 'poster' ) );
	add_settings_field('endofvideooverlay', __('End of video image:'), 'kgvid_endofvideooverlay_callback', __FILE__, 'kgvid_video_embed_playback_settings' );
	add_settings_field('watermark', __('Watermark overlay:'), 'kgvid_watermark_callback', __FILE__, 'kgvid_video_embed_playback_settings', array( 'label_for' => 'watermark' ) );
	add_settings_field('align', __('Video alignment:'), 'kgvid_align_callback', __FILE__, 'kgvid_video_embed_playback_settings', array( 'label_for' => 'align' ) );
	add_settings_field('resize', __('Automatically resize videos:'), 'kgvid_resize_callback', __FILE__, 'kgvid_video_embed_playback_settings', array( 'label_for' => 'inline' ) );
	add_settings_field('inline', __('Inline videos:'), 'kgvid_inline_callback', __FILE__, 'kgvid_video_embed_playback_settings', array( 'label_for' => 'inline' ) );
	add_settings_field('dimensions', __('Max embedded video dimensions:'), 'kgvid_dimensions_callback', __FILE__, 'kgvid_video_embed_playback_settings', array( 'label_for' => 'width' ) );
	add_settings_field('gallery_dimensions', __('Max gallery video dimensions:'), 'kgvid_gallery_dimensions_callback', __FILE__, 'kgvid_video_embed_playback_settings', array( 'label_for' => 'gallery_width' ) );
	add_settings_field('gallery_thumb', __('Gallery thumbnail width:'), 'kgvid_gallery_thumb_callback', __FILE__, 'kgvid_video_embed_playback_settings', array( 'label_for' => 'gallery_thumb' ) );
	add_settings_field('controlbar_style', __('Controlbar style:'), 'kgvid_controlbar_style_callback', __FILE__, 'kgvid_video_embed_playback_settings', array( 'label_for' => 'controlbar_style' ) );
	add_settings_field('autoplay', __('Autoplay:'), 'kgvid_autoplay_callback', __FILE__, 'kgvid_video_embed_playback_settings', array( 'label_for' => 'autoplay' ) );
	add_settings_field('loop', _x('Loop:', 'verb'), 'kgvid_loop_callback', __FILE__, 'kgvid_video_embed_playback_settings', array( 'label_for' => 'loop' ) );
	add_settings_field('preload', __('Preload:'), 'kgvid_preload_callback', __FILE__, 'kgvid_video_embed_playback_settings', array( 'label_for' => 'preload' ) );
	add_settings_field('js_skin', _x('Skin Class:', 'CSS class for video skin'), 'kgvid_js_skin_callback', __FILE__, 'kgvid_video_embed_playback_settings', array( 'label_for' => 'js_skin' ) );

	add_settings_field('bgcolor', __('Background color:'), 'kgvid_bgcolor_callback', __FILE__, 'kgvid_video_embed_flash_settings', array( 'label_for' => 'bgcolor' ) );
	add_settings_field('configuration', __('XML configuration file:'), 'kgvid_configuration_callback', __FILE__, 'kgvid_video_embed_flash_settings', array( 'label_for' => 'configuration' ) );
	add_settings_field('skin', __('Video skin file:'), 'kgvid_skin_callback', __FILE__, 'kgvid_video_embed_flash_settings', array( 'label_for' => 'skin' ) );
	add_settings_field('stream_type', __('Video stream type:'), 'kgvid_stream_type_callback', __FILE__, 'kgvid_video_embed_flash_settings', array( 'label_for' => 'stream_type' ) );
	add_settings_field('scale_mode', __('Video scale mode:'), 'kgvid_scale_mode_callback', __FILE__, 'kgvid_video_embed_flash_settings', array( 'label_for' => 'scale_mode' ) );
	add_settings_field('autohide', __('Autohide:'), 'kgvid_autohide_callback', __FILE__, 'kgvid_video_embed_flash_settings', array( 'label_for' => 'autohide' ) );
	add_settings_field('playbutton', __('Play button overlay:'), 'kgvid_playbutton_callback', __FILE__, 'kgvid_video_embed_flash_settings', array( 'label_for' => 'playbutton' ) );

	add_settings_field('generate_thumbs', __('Default number of thumbnails to generate:'), 'kgvid_generate_thumbs_callback', __FILE__, 'kgvid_video_embed_plugin_settings', array( 'label_for' => 'generate_thumbs' ) );
	add_settings_field('user_roles', __('User roles:'), 'kgvid_user_roles_callback', __FILE__, 'kgvid_video_embed_plugin_settings', array( 'label_for' => 'user_roles' ) );
	add_settings_field('security', __('Video security:'), 'kgvid_security_callback', __FILE__, 'kgvid_video_embed_plugin_settings', array( 'label_for' => 'embeddable' ) );
	add_settings_field('featured', __('Featured image:'), 'kgvid_featured_callback', __FILE__, 'kgvid_video_embed_plugin_settings', array( 'label_for' => 'featured' ) );
	add_settings_field('thumb_parent', __('Attach thumbnails to:'), 'kgvid_thumb_parent_callback', __FILE__, 'kgvid_video_embed_plugin_settings', array( 'label_for' => 'thumb_parent' ) );
	add_settings_field('delete_children', __('Delete associated attachments:'), 'kgvid_delete_children_callback', __FILE__, 'kgvid_video_embed_plugin_settings', array( 'label_for' => 'delete_children' ) );
	add_settings_field('titlecode', __('Video title text HTML formatting:'), 'kgvid_titlecode_callback', __FILE__, 'kgvid_video_embed_plugin_settings', array( 'label_for' => 'titlecode' ) );
	add_settings_field('template', __('Attachment template display:'), 'kgvid_template_callback', __FILE__, 'kgvid_video_embed_plugin_settings', array( 'label_for' => 'template' ) );

	add_settings_field('app_path', __('Path to applications folder on server:'), 'kgvid_app_path_callback', __FILE__, 'kgvid_video_embed_encode_settings', array( 'label_for' => 'app_path' ) );
	add_settings_field('video_app', __('Application for thumbnails & encoding:'), 'kgvid_video_app_callback', __FILE__, 'kgvid_video_embed_encode_settings', array( 'label_for' => 'video_app' ) );
	add_settings_field('browser_thumbnails', __('Enable in-browser thumbnails:'), 'kgvid_browser_thumbnails_callback', __FILE__, 'kgvid_video_embed_encode_settings', array( 'label_for' => 'browser_thumbnails' ) );
	add_settings_field('encode_formats', __('Default video encode formats:'), 'kgvid_encode_formats_callback', __FILE__, 'kgvid_video_embed_encode_settings');
	add_settings_field('automatic', __('Do automatically on upload:'), 'kgvid_automatic_callback', __FILE__, 'kgvid_video_embed_encode_settings', array( 'label_for' => 'auto_encode' ) );
	add_settings_field('htaccess', __('htaccess login:'), 'kgvid_htaccess_callback', __FILE__, 'kgvid_video_embed_encode_settings', array( 'label_for' => 'htaccess_username' ) );
	add_settings_field('ffmpeg_watermark', __('Add watermark to encoded files:'), 'kgvid_ffmpeg_watermark_callback', __FILE__, 'kgvid_video_embed_encode_settings', array( 'label_for' => 'ffmpeg_watermark' ) );
	add_settings_field('moov', __('Method to fix encoded H.264 headers for streaming:'), 'kgvid_moov_callback', __FILE__, 'kgvid_video_embed_encode_settings', array( 'label_for' => 'moov' ) );
	add_settings_field('rate_control', __('Encode quality control method:'), 'kgvid_rate_control_callback', __FILE__, 'kgvid_video_embed_encode_settings', array( 'label_for' => 'rate_control' ) );
	add_settings_field('CRFs', __('Constant Rate Factors (CRF):'), 'kgvid_CRF_options_callback', __FILE__, 'kgvid_video_embed_encode_settings', array( 'label_for' => 'h264_CRF' ) );
	add_settings_field('bitrate_multiplier', __('Average Bit Rate:'), 'kgvid_average_bitrate_callback', __FILE__, 'kgvid_video_embed_encode_settings', array( 'label_for' => 'bitrate_multiplier' ) );
	add_settings_field('h264_profile', __('H.264 profile:'), 'kgvid_h264_profile_callback', __FILE__, 'kgvid_video_embed_encode_settings', array( 'label_for' => 'h264_profile' ) );
	add_settings_field('audio_bitrate', __('Audio bit rate:'), 'kgvid_audio_bitrate_options_callback', __FILE__, 'kgvid_video_embed_encode_settings', array( 'label_for' => 'audio_bitrate' ) );
	add_settings_field('ffmpeg_options', __('FFMPEG legacy options:'), 'kgvid_ffmpeg_options_callback', __FILE__, 'kgvid_video_embed_encode_settings', array( 'label_for' => 'video_bitrate_flag' ) );
	add_settings_field('execution', _x('Execution:', 'program execution options'), 'kgvid_execution_options_callback', __FILE__, 'kgvid_video_embed_encode_settings', array( 'label_for' => 'threads' ) );
...
//callback functions generating HTML for the settings form
	...
	function kgvid_plugin_playback_settings_section_callback() {
		...
		$items = array( "Video.js", "Strobe Media Playback ".__('(deprecated)') );
		if ( $wp_version >= 3.6 ) { $items = array("Video.js" => "Video.js", __("WordPress Default") => "WordPress Default", "Strobe Media Playback ".__('(deprecated)') => "Strobe Media Playback"); }
		echo "<table class='form-table'><tbody><tr valign='middle'><th scope='row'><label for='embed_method'>".__('Video player:')."</label></th><td><select class='affects_player' onchange='kgvid_hide_plugin_settings();' id='embed_method' name='kgvid_video_embed_options[embed_method]'>";
		foreach($items as $name => $value) {
			$selected = ($options['embed_method']==$value) ? 'selected="selected"' : '';
			echo "<option value='$value' $selected>$name</option>";
		}
		echo "</select> <a class='kgvid_tooltip' href='javascript:void(0);'><img src='../wp-includes/images/blank.gif'><span class='kgvid_tooltip_classic'>".__('Video.js is the default player. Users running WordPress 3.6 or higher can choose the WordPress Default Mediaelement.js player which may already be skinned to match your theme. In the past this plugin used Adobe\'s Strobe Media Playback Flash player, which hasn\'t been updated since 2011 and is not recommended.')."</span></a></td></tr></tbody></table>\n";

		$sampleheight = intval($options['height']) + 25;
		echo "<div class='kgvid_setting_nearvid' style='width:".$options['width']."px;'>";
		echo "<div style='float:left;'><input class='affects_player' ".checked( $options['overlay_title'], "on", false )." id='overlay_title' name='kgvid_video_embed_options[overlay_title]' type='checkbox' /> <label for='overlay_title'>".__('Overlay video title')."</label></div>";
		echo "<div style='float:right;'><input class='affects_player' ".checked( $options['overlay_embedcode'], "on", false )." id='overlay_embedcode' name='kgvid_video_embed_options[overlay_embedcode]' type='checkbox' ".$embed_disabled."/> <label for='overlay_embedcode'>".__('Overlay embed code')."</label></div>";
		...
		echo "<div style='float:right;'><input class='affects_player' ".checked( $options['view_count'], "on", false )." id='view_count' name='kgvid_video_embed_options[view_count]' type='checkbox' /> <label for='view_count'>".__('Show view count')."</label></div>";
		echo "</div>\n\t";
	}
	...
	function kgvid_poster_callback() {
		$options = get_option('kgvid_video_embed_options');
		echo "<input class='regular-text affects_player' id='poster' name='kgvid_video_embed_options[poster]' type='text' value='".$options['poster']."' /> <span id='pick-thumbnail' class='button-secondary' data-choose='".__('Choose a Thumbnail')."' data-update='".__('Set as video thumbnail')."' data-change='poster' onclick='kgvid_pick_image(this);'>".__('Choose from Library')."</span>\n\t";
	}

	function kgvid_endofvideooverlay_callback() {
		$options = get_option('kgvid_video_embed_options');
		echo "<input class='affects_player' ".checked( $options['endofvideooverlaysame'], "on", false )." id='endofvideooverlaysame' name='kgvid_video_embed_options[endofvideooverlaysame]' type='checkbox' /> <label for='endofvideooverlaysame'>".__('Display thumbnail image again when video ends.')."</label><br />";
		echo "<input class='regular-text affects_player' id='endofvideooverlay' name='kgvid_video_embed_options[endofvideooverlay]' ".disabled( $options['endofvideooverlaysame'], "true", false )." type='text' value='".$options['endofvideooverlay']."' /> <span id='pick-endofvideooverlay' class='button-secondary' data-choose='".__('Choose End of Video Image')."' data-update='".__('Set as end of video image')."' data-change='endofvideooverlay' onclick='kgvid_pick_image(this);'>".__('Choose from Library')."</span><br />";
		echo "Display alternate image when video ends.<small>\n\t";
	}

	function kgvid_watermark_callback() {
		$options = get_option('kgvid_video_embed_options');
		echo "<input class='regular-text affects_player' id='watermark' name='kgvid_video_embed_options[watermark]' type='text' value='".$options['watermark']."' /> <span id='pick-watermark' class='button-secondary' data-choose='".__('Choose a Watermark')."' data-update='".__('Set as watermark')."' data-change='watermark' onclick='kgvid_pick_image(this);'>".__('Choose from Library')."</span>\n\t";
	}

	function kgvid_align_callback() {
		$options = get_option('kgvid_video_embed_options');
		$items = array(__("left") => "left", __("center") => "center", __("right") => "right");
	...
	function kgvid_resize_callback() {
		$options = get_option('kgvid_video_embed_options');
		echo "<input ".checked( $options['resize'], "on", false )." id='resize' name='kgvid_video_embed_options[resize]' type='checkbox' /> <label for='resize'>".__('Make video player responsive.')."</label>\n\t";
	}

	function kgvid_inline_callback() {
		$options = get_option('kgvid_video_embed_options');
		echo "<input ".checked( $options['inline'], "on", false )." id='inline' name='kgvid_video_embed_options[inline]' type='checkbox' /> <label for='inline'>".__('Allow other content on the same line as the video.')."</label>\n\t";
	}

	function kgvid_dimensions_callback() {
		$options = get_option('kgvid_video_embed_options');
		echo __('Width:')." <input class='small-text affects_player' id='width' name='kgvid_video_embed_options[width]' type='text' value='".$options['width']."' /> ".__('Height:')." <input class='small-text affects_player' id='height' name='kgvid_video_embed_options[height]' type='text' value='".$options['height']."' /> ";
		echo "<input ".checked( $options['minimum_width'], "on", false )." id='minimum_width' name='kgvid_video_embed_options[minimum_width]' type='checkbox' /> <label for='minimum_width'>".__('Enlarge lower resolution videos to max width.')."</label> <a class='kgvid_tooltip' href='javascript:void(0);'><img src='../wp-includes/images/blank.gif'><span class='kgvid_tooltip_classic'>".__('Usually if a video\'s resolution is less than the max width, the video player is set to the actual width of the video. Enabling this will always set the same width regardless of the quality of the video. When necessary you can override by setting the dimensions manually.')."</span></a>\n\t";
	}

	function kgvid_gallery_dimensions_callback() {
		$options = get_option('kgvid_video_embed_options');
		echo __('Width:')." <input class='small-text' id='gallery_width' name='kgvid_video_embed_options[gallery_width]' type='text' value='".$options['gallery_width']."' /> ".__('Height:')." <input class='small-text' id='gallery_height' name='kgvid_video_embed_options[gallery_height]' type='text' value='".$options['gallery_height']."' />\n\t";
	}
	...
	function kgvid_controlbar_style_callback() {
		$options = get_option('kgvid_video_embed_options');
		$items = array(__("docked") => "docked", __("floating") => "floating", __("none") => "none");
		...
		echo "</select> ".__('"Floating" option only works with Strobe Media Playback.')."\n\t";
	}

	function kgvid_autoplay_callback() {
		$options = get_option('kgvid_video_embed_options');
		echo "<input class='affects_player' ".checked( $options['autoplay'], "on", false )." id='autoplay' name='kgvid_video_embed_options[autoplay]' type='checkbox' /> <label for='autoplay'>".__('Play automatically when page loads.')."</label>\n\t";
	}

	function kgvid_loop_callback() {
		$options = get_option('kgvid_video_embed_options');
		echo "<input class='affects_player' ".checked( $options['loop'], "on", false )." id='loop' name='kgvid_video_embed_options[loop]' type='checkbox' /> <label for='loop'>".__('Loop to beginning when video ends.')."</label>\n\t";
	}
	...
	function kgvid_preload_callback() {
		...
		echo "</select><a class='kgvid_tooltip' href='javascript:void(0);'><img src='../wp-includes/images/blank.gif'><span class='kgvid_tooltip_classic'>"._x('Controls how much of a video to load before the user starts playback. Mobile browsers never preload any video information. Selecting "metadata" will load the height and width and format information along with a few seconds of the video in some desktop browsers. "Auto" will preload nearly a minute of video in most desktop browsers. "None" will prevent all data from preloading.', 'Suggest not translating the words in quotation marks')."</span></a>\n\t";
	}

	function kgvid_js_skin_callback() {
		$options = get_option('kgvid_video_embed_options');
		echo "<input class='regular-text code affects_player' id='js_skin' name='kgvid_video_embed_options[js_skin]' type='text' value='".$options['js_skin']."' /><br /><em><small>".sprintf( __('Use %s for a nice, circular play button. Leave blank for the default square play button.')." <a href='http://videojs.com/docs/skins/'>".__('Or build your own CSS skin.'), '<code>kg-video-js-skin</code>')."</a></small></em>\n\t";
	}
	...
	function kgvid_skin_callback() {
		$options = get_option('kgvid_video_embed_options');
		echo "<input class='regular-text affects_player' id='skin' name='kgvid_video_embed_options[skin]' type='text' value='".$options['skin']."' /><br /><em><small>".sprintf( __('Use %s for a modern, circular play button.'), "<code>".plugins_url("", __FILE__)."/flash/skin/kgvid_skin.xml</code>" )."<br /> ".__('Leave blank for the off-center square play button.')."</small></em>\n\t";
	}
	...
	function kgvid_scale_mode_callback() {
		$options = get_option('kgvid_video_embed_options');
		$items = array(__("letterbox") => "letterbox", __("none") => "none", __("stretch") => "stretch", __("zoom") => "zoom");
	...
	function kgvid_autohide_callback() {
		$options = get_option('kgvid_video_embed_options');
		echo "<input ".checked( $options['autohide'], "on", false )." id='autohide' name='kgvid_video_embed_options[autohide]' type='checkbox' /> <label for='autohide'>".__('Autohide controlbar.')."</label>\n\t";
	}

	function kgvid_playbutton_callback() {
		$options = get_option('kgvid_video_embed_options');
		echo "<input class='affects_player' ".checked( $options['playbutton'], "on", false )." id='playbutton' name='kgvid_video_embed_options[playbutton]' type='checkbox' /> <label for='playbutton'>".__('Overlay play button on poster frame.')."</label>\n\t";
	}
	...
	function kgvid_user_roles_callback() {
		...
		$capabilities = array( 'make_video_thumbnails'=>__('Can make thumbnails'), 'encode_videos'=>__('Can encode videos'), 'edit_others_video_encodes' => __('Can view & modify other users encode queue') );
	...
	function kgvid_thumb_parent_callback() {
		...
		echo "</select> <a class='kgvid_tooltip' href='javascript:void(0);'><img src='../wp-includes/images/blank.gif'><span class='kgvid_tooltip_classic'>".__('This depends on your theme. Thumbnails generated by the plugin can be saved as children of the video attachment or the post. Some themes use an image attached to a post instead of the built-in featured image meta tag. Version 3.x of this plugin saved all thumbnails as children of the video.')."</span></a><br /> <a class='button-secondary' href='javascript:void(0);' onclick='kgvid_switch_parents();'>".__('Set all parents')."</a> <a class='kgvid_tooltip' href='javascript:void(0);'><img src='../wp-includes/images/blank.gif'><span class='kgvid_tooltip_classic'>".__('If you\'ve generated thumbnails before changing this option, this will set all existing thumbnails as children of your currently selected option.')."</span></a>\n\t";
	}
	...
	function kgvid_delete_children_callback() {
		...
		echo "</select> <a class='kgvid_tooltip' href='javascript:void(0);'><img src='../wp-includes/images/blank.gif'><span class='kgvid_tooltip_classic'>".__('If you delete the original video you can choose to delete all associated attachments (thumbnails & videos) or keep the thumbnail.')."</span></a>\n\t";
	}

	function kgvid_titlecode_callback() {
		$options = get_option('kgvid_video_embed_options');
		echo "<input class='regular-text code' id='titlecode' name='kgvid_video_embed_options[titlecode]' type='text' value='".htmlentities(stripslashes($options['titlecode']))."' /> <a class='kgvid_tooltip' href='javascript:void(0);'><img src='../wp-includes/images/blank.gif'><span class='kgvid_tooltip_classic'>".sprintf( __('HTML tag applied to titles inserted above the video. Examples: %s. Corresponding closing tags will be applied to the end of the title automatically.'), "&lt;strong&gt;, &lt;em&gt;, &lt;H2&gt;, &lt;span class='videotitle'&gt;")."</span></a>\n\t";
	}

	function kgvid_template_callback() {
		$options = get_option('kgvid_video_embed_options');
		$items = array(__("Video title (WP default)")=>"none", __("Video in existing template")=>"gentle", __("Video only (deprecated)")=>"old");
		echo "<select id='template' name='kgvid_video_embed_options[template]'>";
		foreach($items as $name => $value) {
			$selected = ($options['template']==$value) ? 'selected="selected"' : '';
			echo "<option value='$value' $selected>$name</option>";
		}
		echo "</select> <a class='kgvid_tooltip' href='javascript:void(0);'><img src='../wp-includes/images/blank.gif'><span class='kgvid_tooltip_classic'>".__('By default WordPress only displays a video\'s title on the attachment page. This plugin can filter your attachment page to display the video, or completely replace your attachment template to show only the video. If you were one of the few people using iframe embed codes before version 4.0 of this plugin then you should continue to use "Video only" but otherwise it\'s not recommended.')."</span></a>\n\t";
	}

	function kgvid_app_path_callback() {
		$options = get_option('kgvid_video_embed_options');
		echo "<input class='affects_ffmpeg regular-text code' id='app_path' data-ffmpeg_exists='".$options['ffmpeg_exists']."' name='kgvid_video_embed_options[app_path]' type='text' value='".$options['app_path']."' /><a class='kgvid_tooltip' href='javascript:void(0);'><img src='../wp-includes/images/blank.gif'><span class='kgvid_tooltip_classic'>".sprintf( __('This should be the folder where applications are installed on your server, not a direct path to an application, so it doesn\'t usually end with %1$s. Example: %2$s.'), "<code><strong class='video_app_name'>".strtoupper($options['video_app'])."</strong></code>", "<code>/usr/local/bin</code>" )."\n\t";
	}
	...
	function kgvid_encode_formats_callback() {
		$options = get_option('kgvid_video_embed_options');
		echo "<div class='kgvid_video_app_required'>";
		echo "<input ".checked( $options['encode_fullres'], "on", false )." id='encode_fullres' name='kgvid_video_embed_options[encode_fullres]' type='checkbox' /> <label for='encode_fullres'>".__('Replace original with same resolution H.264')."</label> <a class='kgvid_tooltip' href='javascript:void(0);'><img src='../wp-includes/images/blank.gif'><span class='kgvid_tooltip_classic'>".__('If you have FFMPEG/LIBAV and the proper libraries installed, you can choose to replace your uploaded video with an H.264 video, and also transcode into as many as five additional formats depending on the resolution of your original source. Different browsers have different playback capabilities. Most desktop browsers can play H.264, and all modern mobile devices can play at least 480p H.264. If you create multiple H.264 resolutions, the highest resolution supported by the device will be served up automatically. The plugin will not upconvert your video, so if you upload a 720p video, it will not waste your time creating a 1080p version. There was a time when it seemed like a good idea to provide OGV or WEBM for some desktop browsers, but even Firefox allows H.264 playback on Windows now. I no longer recommend encoding OGV or WEBM unless you expect a large number of no-Flash sticklers visiting your site.')."</span></a><br />";
		echo "<input ".checked( $options['encode_1080'], "on", false )." id='encode_1080' name='kgvid_video_embed_options[encode_1080]' type='checkbox' /> <label for='encode_1080'>1080p H.264 <small><em>".__('(iPhone 4s+, iPad 2+, modern Android, Windows Phone 8, Chrome, Safari, IE 9+, Firefox Windows)')."</em></small></label><br />";
		echo "<input ".checked( $options['encode_720'], "on", false )." id='encode_720' name='kgvid_video_embed_options[encode_720]' type='checkbox' /> <label for='encode_720'>720p H.264 <small><em>".__('(iPhone 4+, iPad, most Android, Chrome, Safari, IE 9+, Firefox Windows)')."</em></small></label><br />";
	...
	function kgvid_automatic_callback() {
		$options = get_option('kgvid_video_embed_options');
		echo "<div class='kgvid_video_app_required'>";
		echo "<input ".checked( $options['auto_encode'], "on", false )." id='auto_encode' name='kgvid_video_embed_options[auto_encode]' type='checkbox' /> <label for='auto_encode'>".__('Encode formats selected above.')."</label><br />";
		...
		echo "<input ".checked( $options['auto_thumb'], "on", false )." id='auto_thumb' name='kgvid_video_embed_options[auto_thumb]' type='checkbox' /> <label for='auto_thumb'>".sprintf( __('Generate thumbnail from %s%% of the way through the video.'), $percentage_select )." </label>";
		echo "</div>\n\t";
	}
	...
	function kgvid_htaccess_callback() {
		$options = get_option('kgvid_video_embed_options');
		echo "<div class='kgvid_video_app_required'>";
		echo "<table class='kgvid_htaccess'><tbody><tr><td>".__('Username:')."</td><td><input class='regular-text affects_ffmpeg' id='htaccess_login' name='kgvid_video_embed_options[htaccess_login]' type='text' value='".$options['htaccess_login']."' /></td></tr>";
		echo "<tr><td>".__('Password:')."</td><td><input class='regular-text affects_ffmpeg' id='htaccess_password' name='kgvid_video_embed_options[htaccess_password]' type='text' value='".$options['htaccess_password']."' /> <a class='kgvid_tooltip' href='javascript:void(0);'><img src='../wp-includes/images/blank.gif'><span class='kgvid_tooltip_classic'>".sprintf( __('If your videos are htaccess protected, %s will access them using these credentials.'), "<strong class='video_app_name'>".strtoupper($options['video_app'])."</strong>" )."</span></a></td></tr></tbody></table>";
		echo "</div>\n\t";
	}

	function kgvid_ffmpeg_watermark_callback() {
		$options = get_option('kgvid_video_embed_options');
		echo "<p><input class='regular-text affects_ffmpeg' id='ffmpeg_watermark_url' name='kgvid_video_embed_options[ffmpeg_watermark][url]' type='text' value='".$options['ffmpeg_watermark']['url']."' /> <span id='pick-ffmpeg-watermark' class='button-secondary' data-choose='".__('Choose a Watermark')."' data-update='".__('Set as watermark')."' data-change='ffmpeg_watermark_url' onclick='kgvid_pick_image(this);'>".__('Choose from Library')."</span></p>\n\t";
		echo "<p>".sprintf( __('Scale: %s%% of video covered by the watermark.'), "<input class='small-text affects_ffmpeg' id='ffmpeg_watermark_scale' name='kgvid_video_embed_options[ffmpeg_watermark][scale]' type='text' value='".$options['ffmpeg_watermark']['scale']."' maxlength='3' />" )."</p>";
		$items = array(__("left")=>"left", _x("center", "horizontal center")=>"center", __("right")=>"right");
		echo "<p>".__('Horizontal align:');
	...
	function kgvid_moov_callback() {
		$options = get_option('kgvid_video_embed_options');
		echo "<div class='kgvid_video_app_required'>";
		$items = array(__("none")=>"none", "movflags faststart"=>"movflag", "qt-faststart"=>"qt-faststart", "MP4Box"=>"MP4Box");
		...
		echo "</select> <a class='kgvid_tooltip' href='javascript:void(0);'><img src='../wp-includes/images/blank.gif'><span class='kgvid_tooltip_classic'>".sprintf( __('By default %1$s places moov atoms at the end of H.264 encoded files, which forces the entire file to download before playback can start and can prevent Flash players from playing them at all. Since approximately October 2012 %1$s can fix the problem at the end of the encoding process by using the option `movflags faststart`. This is the easiest and fastest way to correct the problem, but older versions of %1$s will not work if you select the movflags option. If you can\'t update to a new version of %1$s, select qt-faststart or MP4Box which will run after encoding is finished if they are installed on your server.'), "<strong class='video_app_name'>".strtoupper($options['video_app'])."</strong>")."</span></a>";
		echo "</div>\n\t";
	}
	...
	function kgvid_CRF_options_callback() {
		...
		echo "</select> H.264 <a class='kgvid_tooltip' href='javascript:void(0);'><img src='../wp-includes/images/blank.gif'><span class='kgvid_tooltip_classic'>".__('Lower values are higher quality. 18 is considered visually lossless. Default is 23.')."</span></a><br />";
		...
		echo "</select> WEBM <a class='kgvid_tooltip' href='javascript:void(0);'><img src='../wp-includes/images/blank.gif'><span class='kgvid_tooltip_classic'>".__('Lower values are higher quality. Default is 10.')."</span></a><br />\n\t";
		...
		echo "</select> OGV (qscale) <a class='kgvid_tooltip' href='javascript:void(0);'><img src='../wp-includes/images/blank.gif'><span class='kgvid_tooltip_classic'>".__('Higher values are higher quality. Default is 6.')."</span></a>";
		echo "</div>\n\t";
	}
	...
	function kgvid_average_bitrate_callback() {
		...
		echo "</select> ".__('bits per pixel.')." <a class='kgvid_tooltip' href='javascript:void(0);'><img src='../wp-includes/images/blank.gif'><span class='kgvid_tooltip_classic'>".__('Default is 0.1')."</span></a><br />";
	...
	function kgvid_h264_profile_callback() {
		...
		echo "</select> "._x('profile', 'H.264 profile. Might not need translation')." <a class='kgvid_tooltip' href='javascript:void(0);'><img src='../wp-includes/images/blank.gif'><span class='kgvid_tooltip_classic'>".__('Lower profiles will slightly increase file sizes. This mostly depends on your need for compatability with Android devices. Main profile seems to work on recent phones, although officially Android only supports baseline. High profile is not recommended for mobile or Flash compatibility, and anything above high is designed for professional video and probably incompatible with consumer devices. Older versions of FFMPEG might ignore this setting altogether.')."</span></a><br />";
		...
		echo "</select> "._x('level', 'H.264 level. Might not need translation')." <a class='kgvid_tooltip' href='javascript:void(0);'><img src='../wp-includes/images/blank.gif'><span class='kgvid_tooltip_classic'>".__('3.0 is default. Lower levels will lower maximum bit rates and decoding complexity. This mostly depends on your need for compatability with mobile devices. Older versions of FFMPEG might ignore this setting altogether.')."</span></a>";
		echo "</div>\n\t";
	}
	...
	function kgvid_execution_options_callback() {
		...
		echo "</select> "._x('threads', 'CPU threads. Might not need translating')." <a class='kgvid_tooltip' href='javascript:void(0);'><img src='../wp-includes/images/blank.gif'><span class='kgvid_tooltip_classic'>".sprintf( __('Default is 1, which limits encoding speed but prevents encoding from using too many system resources. Selecting 0 will allow %1$s to optimize the number of threads or you can set the number manually. This may lead to %1$s monopolizing system resources.'), "<strong class='video_app_name'>".strtoupper($options['video_app'])."</strong>" )."</span></a><br />";
		echo "<input ".checked( $options['nice'], "on", false )." id='nice' name='kgvid_video_embed_options[nice]' class='affects_ffmpeg' type='checkbox' /> <label for='nice'>"._x('Run', 'execute program')."  <code>nice</code>.</label> <a class='kgvid_tooltip' href='javascript:void(0);'><img src='../wp-includes/images/blank.gif'><span class='kgvid_tooltip_classic'>".sprintf( __('Tells %1$s to run at a lower priority to avoid monopolizing system resources.'), "<strong class='video_app_name'>".strtoupper($options['video_app'])."</strong>" )."</span></a>";
		echo "</div>";
		...
		echo "<p>".sprintf( __('%s test output:'), "<strong class='video_app_name'>".strtoupper($options['video_app'])."</strong>")."<br /><textarea id='ffmpeg_output' class='ffmpeg_sample_code code' cols='100' rows='20' wrap='soft' readonly='yes'></textarea><br>".sprintf( __('For help interpreting this output, %s try our Wiki page on Github'), "<a href='https://github.com/kylegilman/video-embed-thumbnail-generator/wiki/Interpreting-FFMPEG-or-LIBAV-messages'>")."</a>.</p></div>\n\t";
	}
...
function kgvid_video_embed_options_validate($input) { //validate & sanitize input from settings form
	...
	if (isset ($_POST["video-embed-thumbnail-generator-reset"])) {
		...
		add_settings_error( __FILE__, "options-reset", __("Video Embed & Thumbnail Generator settings reset to default values."), "updated" );
	}
	...
	if ( $input['app_path'] != $options['app_path'] || $input['video_app'] != $options['video_app'] ) {
		...
		if ( $ffmpeg_info['exec_enabled'] == false ) {
			add_settings_error( __FILE__, "ffmpeg-disabled", sprintf( __('%1$s is disabled in PHP settings. You can embed existing videos and make thumbnails with compatible browsers, but video encoding will not work. Contact your System Administrator to find out if you can enable %1$s.'), $ffmpeg_info['function']), "updated");
			$input['ffmpeg_exists'] = "notinstalled";
		}
		elseif ( $ffmpeg_info['ffmpeg_exists'] == false ) {
			add_settings_error( __FILE__, "ffmpeg-disabled", sprintf( __('%1$s not found at %2$s. You can embed existing videos and make thumbnails with compatible browsers, but video encoding is not possible without %1$s.'), strtoupper($input['video_app']), $input['app_path'] ), "updated");
			$input['ffmpeg_exists'] = "notinstalled";
		}
	...
	if ( empty($input['width']) ) {
		add_settings_error( __FILE__, "width-zero", __("You must enter a value for the maximum video width.") );
		$input['width'] = $options['width'];
	}
	if ( empty($input['height']) ) {
		add_settings_error( __FILE__, "height-zero", __("You must enter a value for the maximum video height.") );
		$input['height'] = $options['height'];
	}
	if ( empty($input['gallery_width']) ) {
		add_settings_error( __FILE__, "gallery-width-zero", __("You must enter a value for the maximum gallery video width.") );
		$input['gallery_width'] = $options['gallery_width'];
	}
	if ( empty($input['gallery_height']) ) {
		add_settings_error( __FILE__, "gallery-height-zero", __("You must enter a value for the maximum gallery video height.") );
...
function kgvid_ajax_save_settings() {
	...
	if ( current_user_can('manage_options') ) {
		...
		if ( strpos($setting, 'capability') !== false || strpos($setting, 'ffmpeg_watermark') !== false ) { $validated_options[$setting] = $value; }
		$arr = array ( "error_message" => $error_message, "validated_value" => $validated_options[$setting], "ffmpeg_exists" => $validated_options['ffmpeg_exists'], "encode_string" => implode('', $encode_string), "app_path" => $validated_options['app_path'] );
		echo json_encode($arr);
		die();
	}
}
...
function kgvid_image_attachment_fields_to_edit($form_fields, $post) {
	...
	|| (strpos(get_post_mime_type( $post->post_parent ), 'video') === false && get_post_meta($post->ID, '_kgflashmediaplayer-externalurl', true) == false))
		...
		$form_fields["views"]["html"] = printf( __('%1$s Starts, %2$s Complete Views'), $starts, $completeviews );
		...
		if ( $options['ffmpeg_exists'] == "notinstalled" ) { $ffmpeg_disabled_text = 'disabled="disabled" title="'.sprintf( __(strtoupper('%1$s not found at %2$s and unable to load video in browser for thumbnail generation.'), $options['video_app']), $options['app_path'] ).'"'; }
		else { $ffmpeg_disabled_text = ""; }
...
display: inline-block;">Loading thumbnail...</span></div>';
		...
		if ( current_user_can('make_video_thumbnails') ) {
			...
			<input type="button" id="attachments-'. $post->ID .'-thumbgenerate" class="button-secondary" value="'._x('Generate', 'Button text. Implied "Generate thumbnails"').'" name="thumbgenerate" onclick="kgvid_generate_thumb('. $post->ID .', \'generate\');" '.$ffmpeg_disabled_text.'/>
			<input type="button" id="attachments-'. $post->ID .'-thumbrandomize" class="button-secondary" value="'._x('Randomize', 'Button text. Implied "Randomize thumbnail generation"').'" name="thumbrandomize" onclick="kgvid_generate_thumb('. $post->ID .', \'random\');" '.$ffmpeg_disabled_text.'/>
			<span style="white-space:nowrap;"><input type="checkbox" id="attachments-'. $post->ID .'-firstframe" name="attachments['.$post->ID.'][kgflashmediaplayer-forcefirst]" onchange="document.getElementById(\''.$field_id['thumbtime'].'\').value =\'\';" value="checked" '.$forcefirstchecked.' '.$ffmpeg_disabled_text.'/> <label for="attachments-'. $post->ID .'-firstframe">'.__('Force 1st frame thumbnail').'</label></span></div>';
		...
		$form_fields["generator"]["label"] = _x("Thumbnails", 'Header for thumbnail section');
		...
		<span id="pick-thumbnail" class="button-secondary" style="margin:10px 0;" data-choose="'.__('Choose a Thumbnail').'" data-update="'.__('Set as video thumbnail').'" data-change="attachments-'. $post->ID .'-kgflashmediaplayer-poster" onclick="kgvid_pick_image(this);">'.__('Choose from Library').'</span><br />
		<input type="checkbox" id="attachments-'. $post->ID .'-featured" name="attachments['.$post->ID.'][kgflashmediaplayer-featured]" '.$featuredchecked.' '.$ffmpeg_disabled_text.'/> <label for="attachments-'. $post->ID .'-featured">'.__('Set thumbnail as featured image').'</label>'.$update_script;
		...
		$form_fields["kgflashmediaplayer-poster"]["helps"] = "<small>".sprintf( __('Leave blank to use %sdefault thumbnail'), "<a href='options-general.php?page=video-embed-thumbnail-generator/video-embed-thumbnail-generator.php' target='_blank'>" )."</a>.</small>";
		...
		$form_fields["kgflashmediaplayer-dimensions"]["html"] = __('Width:').' <input name="attachments['. $post->ID .'][kgflashmediaplayer-width]" id="attachments-'. $post->ID .'-kgflashmediaplayer-width" type="text" value="'.$widthset.'" style="width:50px;" data-minimum="'.$options['minimum_width'].'" onchange="kgvid_set_dimension('.$post->ID.', \'height\', this.value);" onkeyup="kgvid_set_dimension('.$post->ID.', \'height\', this.value);"> '.__('Height:').'
		...
		<label for="attachments-'. $post->ID .'-kgflashmediaplayer-lockaspect"><small>'.__('Lock to aspect ratio').'</small></label>';
		$form_fields["kgflashmediaplayer-dimensions"]["helps"] = "<small>".sprintf( __('Leave blank to use %sdefault dimensions'), "<a href='options-general.php?page=video-embed-thumbnail-generator/video-embed-thumbnail-generator.php' target='_blank'>" )."</a>.</small>";
		...
		foreach ( $track_option as $track => $track_attribute ) {
			$items = array(__("subtitles")=>"subtitles", __("captions")=>"captions", __("chapters")=>"chapters");
			...
			$tracks_html .= '<div id="kgflashmediaplayer-'.$post->ID.'-trackdiv-'.$track.'" class="kgvid_thumbnail_box kgvid_track_box"><strong>'._x('Track', 'Like video track').' '.strval($track+1).'</strong><span class="kgvid_track_box_removeable" onclick="jQuery(this).parent().remove();jQuery(\'form.compat-item input\').first().change();"></span><br />
			'.__('Track type:').' '.$track_type_select.'<br />
			<span id="pick-track'.$track.'" class="button-secondary" style="margin:10px 0;" data-choose="'.__('Choose a Text File').'" data-update="'.__('Set as track source').'" data-change="attachments-'. $post->ID .'-kgflashmediaplayer-track_'.$track.'_src" onclick="kgvid_pick_attachment(this);">'.__('Choose from Library').'</span><br />
			...
			'._x('Language code:', 'two-letter code indicating track\'s language').' <input name="attachments['. $post->ID .'][kgflashmediaplayer-track]['.$track.'][srclang]" id="attachments-'. $post->ID .'-kgflashmediaplayer-track_'.$track.'_srclang" type="text" value="'.$track_option[$track]['srclang'].'" maxlength="2" style="width:40px;"><br />
			'.__('Label:').' <input name="attachments['. $post->ID .'][kgflashmediaplayer-track]['.$track.'][label]" id="attachments-'. $post->ID .'-kgflashmediaplayer-track_'.$track.'_label" type="text" value="'.$track_option[$track]['label'].'" class="text"></div>';
		}
		$tracks_html .= '</div>';

		$form_fields["kgflashmediaplayer-track"]["label"] = __("Subtitles & Captions");
		$form_fields["kgflashmediaplayer-track"]["input"] = "html";
		$form_fields["kgflashmediaplayer-track"]["html"] = $tracks_html.'<span class="button-secondary" id="kgflashmediaplayer-add_track" onclick="kgvid_add_subtitles('.$post->ID.')">'.__('Add track').'</span>';
		...
		$items = array(__("Single Video")=>"Single Video", __("Video Gallery")=>"Video Gallery", __("WordPress Default")=>"WordPress Default");
		...
		<label for="attachments-'.$post->ID.'-kgflashmediaplayer-showtitle">'.__('Insert title above video').'</label><br />
		...
		<label for="attachments-'.$post->ID.'-kgflashmediaplayer-downloadlink">'.__('Insert download link below video').'<em><small><br />'.__('Makes it easier for users to download file.').'</em></small></label><br />
		<label for="attachments-'.$post->ID.'-kgflashmediaplayer-embed">'.__('Insert', 'verb').'</label>
		'.$shortcode_select.'
		...
		if ( get_post_meta($post->ID, "_kgflashmediaplayer-embed", true) == "Video Gallery" ) {
			...
			$form_fields["kgflashmediaplayer-gallery"]["html"] = '<input name="attachments['.$post->ID.'][kgflashmediaplayer-gallery_thumb_width]" id="attachments-'.$post->ID.'-kgflashmediaplayer-gallery_thumb_width" type ="text" value="'.$gallery_thumb_width.'" class="kgvid_50_width"> <label for="attachments-'.$post->ID.'-kgflashmediaplayer-gallery_thumb_width">'.__('Thumbnail Width').'</label><br />
			'.$gallery_orderby_select.' '.__('Order By').'<br />
			'.$gallery_order_select.' '.__('Sort Order').'<br />
			<input name="attachments['.$post->ID.'][kgflashmediaplayer-gallery_exclude]" id="attachments-'.$post->ID.'-kgflashmediaplayer-gallery_exclude" type ="text" value="'.$gallery_exclude.'" class="kgvid_50_width"> <label for="attachments-'.$post->ID.'-kgflashmediaplayer-gallery_exclude">'.__('Exclude').'</label><br />
			<input name="attachments['.$post->ID.'][kgflashmediaplayer-gallery_include]" id="attachments-'.$post->ID.'-kgflashmediaplayer-gallery_include" type ="text" value="'.$gallery_include.'" class="kgvid_50_width"> <label for="attachments-'.$post->ID.'-kgflashmediaplayer-gallery_include">'.__('Include').'</label><br />
			<input name="attachments['.$post->ID.'][kgflashmediaplayer-gallery_id]" id="attachments-'.$post->ID.'-kgflashmediaplayer-gallery_id" type ="text" value="'.$gallery_id.'" class="kgvid_50_width"> <label for="attachments-'.$post->ID.'-kgflashmediaplayer-gallery_id">'.__('Post ID').'</label>
			';
...
function kgvid_save_thumb($post_id, $post_name, $thumb_url, $index=false) {
	...
	elseif ( $success ) { //no existing post with this filename
		...
		$desc = $post_name . ' '._x('thumbnail', 'text appended to newly created thumbnail titles');
		if ( $index ) { $desc .= ' '.$index; }
...
function kgvid_embedurl_menu($tabs) {
	$newtab = array( 'embedurl' => _x('Embed Video from URL', 'Title in "Add Media" popup sidebar') );
...
function media_embedurl_process() {
	...
	<tbody>
				<tr>
					<th valign="top" scope="row" class="label"><span class="alignleft"><label for="videotitle"><?php _e('Video Title'); ?></label></span></th>
					<td class="field"><input type="text" id="videotitle" name="videotitle" value="" size="50" />
					<p class="help"><small><?php _e('Add an optional header above the video.'); ?></small></p></td>
				</tr>
				<tr>
					...
					<p class="help"><small><?php _e('Specify the URL of the video file.'); ?></small></p></td>
				</tr>
				<?php if ( current_user_can('make_video_thumbnails') ) { ?>
				<tr>
					<th valign="top" scope="row" class="label"><span class="alignleft"><label for="numberofthumbs"><?php _e('Thumbnails') ?></label></span></th>
					<td class="field">
						<input id="attachments-singleurl-numberofthumbs" type="text" value="<?php echo $options['generate_thumbs']; ?>" maxlength="2" size="4" style="width:35px;text-align:center;" title="<?php _e('Number of Thumbnails') ?>" onchange="document.getElementById('attachments-singleurl-thumbtime').value='';" />
						<input type="button" id="attachments-singleurl-thumbgenerate" class="button-secondary" value="<?php _e('Generate') ?>" name="thumbgenerate" onclick="kgvid_generate_thumb('singleurl', 'generate');" disabled title="<?php _e('Please enter a valid video URL') ?>" />
						<input type="button" id="attachments-singleurl-thumbrandomize" class="button-secondary" value="<?php _e('Randomize') ?>" name="thumbrandomize" onclick="kgvid_generate_thumb('singleurl', 'random');" disabled title="<?php _e('Please enter a valid video URL') ?>" />
						<input type="checkbox" id="attachments-singleurl-firstframe" onchange="document.getElementById('attachments-singleurl-thumbtime').value ='';" /><label for="attachments-singleurl-firstframe"><?php _e('Force 1st Frame Thumbnail') ?></label><br>
						<div id="attachments-singleurl-thumbnailplaceholder"></div>
						<span><?php _e('Thumbnail timecode:') ?></span> <input name="attachments[singleurl][thumbtime]" id="attachments-singleurl-thumbtime" type="text" value="" style="width:60px;"><br>
						<input type="checkbox" <?php echo checked( $options["featured"], "on", false ); ?> id="attachments-singleurl-featured" name="attachments[singleurl][kgflashmediaplayer-featured]" /> <?php _e('Set thumbnail as featured image') ?>
					</td>
				</tr>
				<?php } ?>
				<tr>
					<th valign="top" scope="row" class="label"><span class="alignleft"><label for="attachments-singleurl_kgflashmediaplayer_poster"><?php _e('Thumbnail URL') ?></label></span></th>
					...
					<p class="help"><small><?php printf( __('Leave blank to use %sdefault thumbnail'), '<a href="options-general.php?page=video-embed-thumbnail-generator/video-embed-thumbnail-generator.php" target="_blank">' ) ?></a>.</small></p></td>
				</tr>
				<tr>
					<th valign="top" scope="row" class="label"><span class="alignleft"><label for="attachments-singleurl-kgflashmediaplayer-width"><?php _e('Dimensions') ?></label></span></th>
					<td class="field"><?php _e('Width:') ?> <input name="attachments[singleurl][kgflashmediaplayer-width]" type="text" value="<?php echo $maxwidth; ?>" id="attachments-singleurl-kgflashmediaplayer-width" type="text" class="kgvid_50_width" onchange="kgvid_set_dimension('singleurl', 'height', this.value);" onkeyup="kgvid_set_dimension('singleurl', 'height', this.value);"> <?php _e('Height:') ?> <input name="attachments[singleurl][kgflashmediaplayer-height]" id="attachments-singleurl-kgflashmediaplayer-height" type="text" value="<?php echo $maxheight; ?>" class="kgvid_50_width" onchange="kgvid_set_dimension('singleurl', 'width', this.value);" onkeyup="kgvid-set-dimension('singleurl', 'width', this.value);"> <input type="checkbox" name="attachments[singleurl][kgflashmediaplayer-lockaspect]" id="attachments-singleurl-kgflashmediaplayer-lockaspect" onclick="kgvid_set_aspect('singleurl', this.checked);" checked> <label for="attachments-singleurl-kgflashmediaplayer-lockaspect"><small><?php _e('Lock to Aspect Ratio') ?></small></label>
					<p class="help"><small><?php printf( __('Leave blank to use %sdefault dimensions'), '<a href="options-general.php?page=video-embed-thumbnail-generator/video-embed-thumbnail-generator.php" target="_blank">' ) ?></a>.</small></p></td>
				</tr>
				<?php if ( current_user_can('encode_videos') ) { ?>
				<tr>
					<th valign="top" scope="row" class="label"><span class="alignleft"><label for="html5"><?php _e('Additional Formats') ?></span></label></th>
					<td><?php echo $checkboxes; ?></td>
				</tr>
				<?php } ?>
				<tr>
					<th valign="top" scope="row" class="label"><span class="alignleft"><label>Options</span></label></th>
					<td><input type="checkbox" name="downloadlink" id="downloadlink" value="true" class="field" /><label for="downloadlink"><?php _e('Generate Download Link Below Video') ?><br /><small></em><?php _e('Makes it easier for users to download video file') ?></em></small></label></td>
				</tr>
				<tr class="submit">
					<td></td>
					<td>
						<input type="button" onclick="kgvid_insert_shortcode();" name="insertonlybutton" id="insertonlybutton" class="button" value="<?php _e('Insert into Post') ?>" disabled title="<?php _e('Please enter a valid video URL') ?>" />
					</td>
				</tr>
	</tbody></table>
	</div>
...
function kgvid_generate_attachment_shortcode($kgvid_video_embed) {
	...
	if (array_key_exists('sample', $kgvid_video_embed)) {
		if ( $options['overlay_title'] == "on" ) { $shortcode .= ' title="'._x('Sample Video', 'Sample like example').'"'; }
		if ( $options['overlay_embedcode'] == "on" ) { $shortcode .= ' embedcode="'.__('Sample Embed Code').'"'; }
		$shortcode .= ' caption="'.__('If text is entered in the attachment\'s caption field it is displayed here automatically.').'"';
	}
...
function kgvid_make_thumbs($postID, $movieurl, $numberofthumbs, $i, $iincreaser, $thumbtimecode, $dofirstframe, $generate_button) {
	...
	if ($movie_info['worked'] == true) { //if FFMPEG was able to open the file
		...
		$thumbnaildisplaycode = "<strong>".__('Can\'t open movie file.')."</strong><br />".$movie_info['output'];
	...
	if ($movie_info['worked'] == true) { //if FFMPEG was able to open the file
		...
		if ( !empty($encode_list) ) {
			...
			if ( $already_queued !== false ) {
				...
				if ( !empty($encode_list) ) { $embed_display = "<strong>".sprintf( __('%1$s updated in existing queue entry in position %2$s.'), $imploded_encode_list, $existing_queue_position )." </strong>"; }
				else { $embed_display = "<strong>".sprintf( __('Video is already queued in position %s.'), $existing_queue_position )." </strong>"; }
			}
			else {
				...
				if ( $queue_position == 0 ) { $embed_display = "<strong>".__('Starting')." ".strtoupper($options['video_app'])."... </strong>"; }
				else { $embed_display = "<strong>".sprintf( __('%1$s added to queue in position %2$s.'), $imploded_encode_list, $new_queue_position )." </strong>";
				}
			}
		} //if any video formats don't already exist, add to queue
		else { $embed_display = "<strong>".__('Nothing to encode.')."</strong>"; }
...
function kgvid_callffmpeg() {
	...
	if ( $options['ffmpeg_exists'] == true ) {
		...
		if ( $action == "submit" && current_user_can('make_video_thumbnails') ) { kgivd_save_singleurl_poster($parent_id, $poster, $movieurl, $set_featured); }
			...
			$thumbnaildisplaycode = '<strong>'.sprintf( __('Error: %1$s not found. Verify that %1$s is installed at %2$s and check the %3$sapplication path plugin setting'), strtoupper($options["video_app"]), strtoupper($options["app_path"]), '<a href="options-general.php?page=video-embed-thumbnail-generator/video-embed-thumbnail-generator.php">' ).'</a>.</strong>' ;
			$arr = array ( "thumbnaildisplaycode"=>$thumbnaildisplaycode, "embed_display"=>$thumbnaildisplaycode, "lastthumbnumber"=>"break" );
			echo json_encode($arr);
	}//no ffmpeg
		...
		if ( !empty($queued_format) ) {
			...
			foreach( $video_formats as $format => $format_stats ) {
				if ( $queued_format == $format && $format_stats['type'] == "h264" ) {
					if ( $movie_info['configuration']['libx264'] == "true" && $aac_available ) {
						...
						else { $embed_display = sprintf( __('%s already encoded'), $format_stats['name'] ); }
						...
						$lastline = sprintf( __('%s missing library libx264 required for H.264 encoding'), strtoupper($options['video_app']) );
						if ( !$aac_available ) {
							...
							$aac_list .= ", ".__('or')." ".$lastaac;
							$lastline .= " ".sprintf( __('and missing an AAC encoding library. Please install and enable libx264 and %s'), $aac_list );
						}
						...
						$embed_display = __("Missing libraries");
					}
			...
			if ( $queued_format == "webm" || $queued_format == "ogg" ) { //if it's not H.264 they both work essentially the same
				if ( ! $encodevideo_info[$queued_format.'_exists'] || ($encodevideo_info['sameserver'] && filesize($encodevideo_info[$queued_format.'filepath']) < 24576) ) {
					if ( $movie_info['configuration']['libvorbis'] == "true" && $movie_info['configuration'][$video_formats[$queued_format]['vcodec']] == "true" ) {
						...
						$embed_display = sprintf( __('Encoding %s'), $video_formats[$queued_format]['name'] );
						...
						$lastline = sprintf( __('%1$s missing library %2$s required for %3$s encoding.'), strtoupper($options['video_app']), implode(', ', $missing_libraries), $video_formats[$queued_format]['name']);
				...
				else { $embed_display = sprintf( __('%s already encoded.'), $video_formats[$queued_format]['vcodec'] ); }
			}//if format is queued
			...
			if ( !empty($encode_string) ) {
				...
				if ( !empty($cmd) ) { $cmd = $cmd.' > "'.$logfile.'" 2>&1 & echo $!'; }
					...
					$arr = array ( "embed_display"=>"<span class='kgvid_warning'>".__("Error: Command 'escapeshellcmd' is disabled on your server.")."</span>" );
					return $arr;
				}
...
function kgvid_encode_progress($video_key, $format, $page) {
	...
	if ( is_array($video_embed_queue) && array_key_exists($video_key, $video_embed_queue) ) {
		...
		if ( $video_entry['encode_formats'][$format]['status'] == "error" ) {
			$embed_display = '<strong>'.__('Error:').' </strong><span class="kgvid_warning">'.stripslashes($video_entry['encode_formats'][$format]['lastline']).'.</span>';
			...
			if ( $video_entry['encode_formats'][$format]['status'] != "Encoding Complete" ) {
				...
				if ( is_file($logfile) ) {
					...
					if ( array_key_exists(1, $time_matches) == true ) { //still encoding
						...
						$embed_display = '<strong>'.__('Encoding').'</strong> <div class="kgvid_meter"><div class="kgvid_meter_bar" style="width:'.$percent_done.'%;"><div class="kgvid_meter_text">'.$percent_done_text.'</div></div></div>';

						if ( current_user_can('encode_videos') && $pid ) {
							$embed_display .= '<a href="javascript:void(0);" class="kgvid_cancel_button" id="attachments-'.$video_entry["attachmentID"].'-kgflashmediaplayer-cancelencode" onclick="kgvid_cancel_encode('.$pid.', \''.$video_entry["attachmentID"].'\', \''.$video_key.'\', \''.$format.'\');">'.__('Cancel').'</a>';
						}

						$embed_display .= '<div style="display:block;"><small>'.__('Elapsed:').' '.date('H:i:s',$time_elapsed).'. '.__('Remaining:').' '.$time_remaining.'. '._x('FPS:', 'Frames per second').' '.$fps_match.'</small></div><script type="text/javascript">percent_timeout = setTimeout(function(){'.$script_function.'}, '.$time_to_wait.');</script>';
					}
					elseif ( time() - $started < 10 || ( file_exists($filepath) && time() - filemtime($filepath) < 10 ) ) { //not enough time has passed, so check again later
						$args = array($video_key, $format, $page);
						$embed_display = '<strong>'.__('Encoding').'</strong> <script type="text/javascript">percent_timeout = setTimeout(function(){'.$script_function.'}, 1000);</script>';
						wp_schedule_single_event(time()+60, 'kgvid_cron_queue_check', $args);
					}
					elseif ( array_key_exists(1, $video_matches) == true || array_key_exists(1, $libx264_matches) == true ) { //encoding complete
						...
						$embed_display = '<strong>'.__('Encoding Complete').'</strong> <script type="text/javascript">percent_timeout = setTimeout(function(){'.$script_function.'}, 1000);</script>';
						$next_video = kgvid_encode_videos(); //start the next queued video
						...
						if ( strpos($lastline, "signal 15") !== false ) { //if the encoding was intentionally canceled
							$lastline = __("Encoding was canceled.");
						}
						$video_embed_queue[$video_key]['encode_formats'][$format]['status'] = "error";
				...
				if ( $video_embed_queue[$video_key]['encode_formats'][$format]['status'] == "error" ) {
					...
					update_option('kgvid_video_embed_queue', $video_embed_queue);
					$embed_display = '<strong>'.__('Error:').' </strong><span class="kgvid_warning">'.stripslashes($lastline).'.</span>';
					$next_video = kgvid_encode_videos(); //start the next queued video
					if ( !empty($next_video['format']) ) {
						$embed_display .= '<script type="text/javascript">percent_timeout = setTimeout(function(){'.$script_function.'}, 1000);</script>';
						$args = array($next_video['video_key'], $next_video['format'], $page);
		...
		else { $arr = array ( "embed_display"=>"<strong>".__('Waiting...')."</strong>" ); }
		return $arr;
...
function kgvid_fix_moov_atom($filepath) {
	...
	if ( $options['moov'] == "qt-faststart" || $options['moov'] == "MP4Box" ) {
		...
		$output = "\n".__('Fixing moov atom for streaming');
...
function kgvid_add_contextual_help_tab() {
	...
	get_current_screen()->add_help_tab( array(
        ...
        'content'   => '<p><strong>'.__('Use these optional attributes in the [KGVID] shortcode:').'</strong></p>
<ul><li><code>id="xxx"</code> '.__('video attachment ID (instead of using a URL).').'</li>
<li><code>videos="x"</code> '.__('number of attached videos to display if no URL or ID is given.').'</li>
<li><code>orderby="menu_order/title/post_date/rand/ID"</code> '.__('criteria for sorting attached videos if no URL or ID is given.').'</li>
<li><code>order="ASC/DESC"</code> '.__('sort order.').'</li>
<li><code>poster="http://www.example.com/image.jpg"</code> '.__('sets the thumbnail.').'</li>
<li><code>endofvideooverlay="http://www.example.com/end_image.jpg</code> '.__('sets the image shown when the video ends.').'</li>
<li><code>width="xxx"</code></li>
<li><code>height="xxx"</code></li>
<li><code>align="left/right/center"</code></li>
<li><code>inline="true/false"</code> '.__('allow other content on the same line as the video').'</li>
<li><code>volume="0.x"</code> '.__('pre-sets the volume for unusually loud videos. Value between 0 and 1.').'</li>
<li><code>controlbar="docked/floating/none"</code> '.__('sets the controlbar position. "Floating" option only works with Strobe Media Playback.').'</li>
<li><code>loop="true/false"</code></li>
<li><code>autoplay="true/false"</code></li>
<li><code>watermark="http://www.example.com/image.png"</code> '.sprintf( __('or %s to disable.'), $false_code ).'</li>
<li><code>title="Video Title"</code> '.sprintf( __('or %s to disable.'), $false_code ).'</li>
<li><code>embedcode="html code"</code> '.sprintf( __('changes text displayed in the embed code overlay in order to provide a custom method for embedding a video or %s to disable.'), $false_code ).'</li>
<li><code>view_count="true/false"</code> '.__('turns the view count on or off.').'</li>
<li><code>caption="Caption"</code> '.__('text that is displayed below the video (not subtitles or closed captioning)').'</li>
<li><code>description="Description"</code> '.__('Used for metadata only.').'</li>
<li><code>downloadlink="true/false"</code> '.__('generates a link below the video to make it easier for users to save the video file to their computers.').'</li>
<li><code>right_click="true/false"</code> '.__('allow or disable right-clicking on the video player.').'</li>
<li><code>resize="true/false"</code> '.__('allow or disable responsive resizing.').'</li></ul>

<p><strong>'.__('These options will add a subtitle/caption track.').'</strong></p>
<ul><li><code>track_src="http://www.example.com/subtitles.vtt_.txt"</code> '.__('URL of the WebVTT file.').'</li>
<li><code>track_kind=subtitles/captions/chapters</code></li>
<li><code>track_srclang=xx</code> '.__('the track\'s two-character language code (en, fr, es, etc)').'</li>
<li><code>track_label="Track Label"</code> '.__('text that will be shown to the user when selecting the track.').'</li></ul>

<p><strong>'.__('These options will only affect Video.js playback').'</strong></p>
<ul><li><code>skin="example-css-class"</code> '.sprintf( __('Completely change the look of the video player. %sInstructions here.'), '<a href="https://github.com/zencoder/video-js/blob/master/docs/skins.md">' ).'</a></li></ul>

<p><strong>'.__('These options will only affect Flash playback in Strobe Media Playback video elements. They will have no effect on other players.').'</p></strong>
<ul><li><code>autohide="true/false"</code> '.__('specify whether to autohide the control bar after a few seconds.').'</li>
<li><code>playbutton="true/false"</code> '.__('turns the big play button overlay in the middle of the video on or off.').'</li>
<li><code>streamtype="live/recorded/DVR"</code> '.__('I honestly don\'t know what this is for.').'</li>
<li><code>scalemode="letterbox/none/stretch/zoom"</code> '.__('If the video display size isn\'t the same as the video file, this determines how the video will be scaled.').'</li>
<li><code>backgroundcolor="'.__('#rrggbb').'"</code> '.__('set the background color to whatever hex code you want.').'</li>
<li><code>configuration="http://www.example.com/config.xml"</code> '.__('Lets you specify all these flashvars in an XML file.').'</li>
<li><code>skin="http://www.example.com/skin.xml"</code> '.sprintf( __('Completely change the look of the video player. %sInstructions here.'), '<a href="http://www.longtailvideo.com/support/jw-player/jw-player-for-flash-v5/14/building-skins">' ).'</a></li></ul>

<p><strong>'.__('These options are available for video galleries (options work the same as standard WordPress image galleries)').'</p></strong>
<ul><li><code>gallery="true"</code> '.__('turns on the gallery').'</li>
<li><code>gallery_thumb="xxx"</code> '.__('width in pixels to display gallery thumbnails').'</li>
<li><code>gallery_exclude="15"</code> '.__('comma separated video attachment IDs. Excludes the videos from the gallery.').'</li>
<li><code>gallery_include="65"</code> '.__('comma separated video attachment IDs. Includes only these videos in the gallery. Please note that include and exclude cannot be used together.').'</li>
<li><code>gallery_orderby="menu_order/title/post_date/rand/ID"</code> '.__('criteria for sorting the gallery').'</li>
<li><code>gallery_order="ASC/DESC"</code> '.__('sort order').'</li>
<li><code>gallery_id="241"</code> '.__('post ID to display a gallery made up of videos associated with a different post.').'</li></ul>'
    ) );
