		...
		<InspectorControls>
			<PanelBody title={ __( 'Gallery Settings' ) }>
				<TextControl
					...
					onChange={ (value) => setAttributes({ gallery_include: value }) }
				/>
				<SelectControl
					__nextHasNoMarginBottom
					__next40pxDefaultSize
					...
					onChange={ (value) => setAttributes({ gallery_orderby: value }) }
				...
				<SelectControl
					...
					label={ __( 'Sort order' ) }
					value={ gallery_order }
					onChange={ (value) => setAttributes({ gallery_order: value }) }
					options={ [
				...
				<ToggleControl
					...
					onChange={ (value) => setAttributes({ gallery_pagination: value }) }
				/>
				{ gallery_pagination && <TextControl
					label={ __( 'Number of videos per page' ) }
					value={ gallery_per_page }
					onChange={ (value) => setAttributes({ gallery_per_page: value }) }
					/>
				}
				<ToggleControl
					__nextHasNoMarginBottom
				...
				<SelectControl
					...
					value={ gallery_end }
					onChange={ (value) => setAttributes({ gallery_end: value }) }
					options={ [
						{ label: __( 'Stop, but leave popup window open' ), value: '' },
						{ label: __( 'Autoplay next video in the gallery' ), value: 'next' },
