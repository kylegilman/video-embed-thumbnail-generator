...
const EncodeFormatStatus = ( {
	...
	let browserEncodingLabel;
	if (
		window.videopack_current_browser_job_id &&
		Number( window.videopack_current_browser_job_id ) ===
			Number( formatData.job_id || formatData.id )
	) {
		browserEncodingLabel = __(
			'Encoding (This Browser Tab)',
			'video-embed-thumbnail-generator'
		);
	} else {
		browserEncodingLabel = __(
			'Encoding (Different Browser/Tab)',
			'video-embed-thumbnail-generator'
		);
	}
			...
			{ showLabel &&
				( !! ffmpegExists ? (
					<CheckboxControl
						...
						onChange={ ( value ) =>
							onCheckboxChange( formatId, value )
						}
					/>
				) : (
