	...
	if ( 'undefined' !== typeof window.mejs ) {
		...
		Object.assign( MediaElementPlayer.prototype, {
			buildspeed: function buildspeed( player, controls, layers, media ) {
				...
				for (
					let i = 0, total = t.options.speeds.length;
					i < total;
					i++
				) {
				...
				player.speedButton.innerHTML =
					...
					`aria-label="${ speedTitle }" tabindex="0">${ getSpeedNameFromValue(
						t.options.defaultSpeed
					) }</button>` +
				...
				for ( let i = 0, total = speeds.length; i < total; i++ ) {
					...
					player.speedButton.querySelector( 'ul' ).innerHTML +=
						...
						`${
							speeds[ i ].value === t.options.defaultSpeed
								? ` ${ t.options.classPrefix }speed-selected`
								: ''
						}">` +
						`${ speeds[ i ].name }</label>` +
						`</li>`;
				...
				player.speedSelector = player.speedButton.querySelector(
					`.${ t.options.classPrefix }speed-selector`
				);

				const inEvents = [ 'mouseenter', 'focusin' ],
					outEvents = [ 'mouseleave', 'focusout' ],
					// Enable inputs after they have been appended to controls to avoid tab and up/down arrow focus issues
					radios = player.speedButton.querySelectorAll(
						'input[type="radio"]'
					),
					labels = player.speedButton.querySelectorAll(
						`.${ t.options.classPrefix }speed-selector-label`
					);
				/**
				...
				for ( let i = 0, total = inEvents.length; i < total; i++ ) {
					player.speedButton.addEventListener( inEvents[ i ], () => {
						...
						player.speedSelector.style.height =
							player.speedSelector.querySelector(
								'ul'
							).offsetHeight;
						player.speedSelector.style.top = `${
							-1 * parseFloat( player.speedSelector.offsetHeight )
						}px`;
					} );
				}
				...
				for ( let i = 0, total = radios.length; i < total; i++ ) {
					...
					radio.addEventListener( 'click', function () {
						...
						player.speedButton.querySelector( 'button' ).innerHTML =
							getSpeedNameFromValue( newSpeed );
						const selected = player.speedButton.querySelectorAll(
							`.${ t.options.classPrefix }speed-selected`
						);
						for (
							let si = 0, selectedTotal = selected.length;
							si < selectedTotal;
							si++
						) {
							mejs.Utils.removeClass(
						...
						self.checked = true;
						const siblings = mejs.Utils.siblings( self, ( el ) =>
							mejs.Utils.hasClass(
								el,
								`${ t.options.classPrefix }speed-selector-label`
							)
						...
						for (
							let j = 0, siblingsTotal = siblings.length;
							j < siblingsTotal;
							j++
						) {
							mejs.Utils.addClass(
				...
				for ( let i = 0, total = labels.length; i < total; i++ ) {
					labels[ i ].addEventListener( 'click', function () {
						const radio = mejs.Utils.siblings(
								this,
								( el ) => el.tagName === 'INPUT'
							)[ 0 ],
				...
				t.options.keyActions.push(
					{
						 ...
						 */
						keys: [ 60, 188 ], // "<" & ","
						action: ( keyPlayer, keyMedia, key, event ) => {
							if ( event.key !== '<' ) {
								return;
							}

							const _radios = keyPlayer.speedRadioButtons;
			...
			cleanspeed: function cleanspeed( player ) {
				if ( player ) {
					if ( player.speedButton ) {
						player.speedButton.parentNode.removeChild(
							player.speedButton
						);
					}
					if ( player.speedSelector ) {
						player.speedSelector.parentNode.removeChild(
							player.speedSelector
						);
					}
				}
		...
		Object.assign( MediaElementPlayer.prototype, {
			...
			buildsourcechooser( player, controls, layers, media ) {
				const t = this,
					sourceTitle = mejs.Utils.isString(
				...
				const flatSources = hasSourceGroups
					? Object.keys( source_groups ).flatMap(
							( groupId ) =>
								source_groups[ groupId ].sources || []
					  )
					: sources;

				if ( hasSourceGroups ) {
					const groups =
						player.groupSourcesByResolution( flatSources );
					...
					groups.forEach( ( { res, candidates } ) => {
						...
						const isCurrent = candidates.some(
							( c ) => c.src === currentSrc
						);
						if ( isCurrent ) {
				...
				player.sourcechooserButton.addEventListener(
					...
					mejs.Utils.debounce( () => {
						...
						setTimeout( () => {
							const parent =
								player.sourcechooserButton.ownerDocument.activeElement.closest(
				...
				player.sourcechooserButton
					...
					.addEventListener( 'click', function () {
						...
						const selector = mejs.Utils.siblings( this, ( el ) =>
							mejs.Utils.hasClass(
								el,
								`${ t.options.classPrefix }sourcechooser-selector`
							)
			...
			groupSourcesByResolution( sources ) {
				...
				return Object.keys( byRes )
					...
					.sort(
						( a, b ) =>
							parseInt( b.res, 10 ) - parseInt( a.res, 10 )
					);
			},
			...
			addSourceButton( src, label, type, isCurrent ) {
				...
				const inputId = `${ t.id }_sourcechooser_${ sourceUrl.replace(
					/[^a-zA-Z0-9]/g,
					''
				) }`;
				...
				li.innerHTML =
					...
					`role="menuitemradio" value="${ sourceUrl }" ${
			...
			performResolutionSwap( videoEl, media, setSourceFn ) {
				...
				if ( currentTime > 0 && videoEl.videoWidth ) {
					...
					canvas.width =
						videoEl.videoWidth || videoEl.offsetWidth || 640;
					canvas.height =
						videoEl.videoHeight || videoEl.offsetHeight || 360;
					...
					try {
						...
						const container =
							this.container && this.container.get
								...
								: this.container;
						( container || videoEl.parentNode ).appendChild(
							canvas
						);
					} catch {
						canvas = null;
			...
			changeRes( src ) {
				...
				if ( t.sourcechooserButton ) {
					const radios =
						t.sourcechooserButton.querySelectorAll(
							'input[type=radio]'
						);
					...
					if ( selectedRadio ) {
						...
						selectedRadio
							.closest( 'li' )
							.classList.add( 'sourcechooser-selected' );
					}
				}

				const candidates =
					t.resolutionCandidates && t.resolutionCandidates[ src ];
			 ...
			 *
			 * @return {string} The current resolution label, or an empty string if unset.
			 */
			getCurrentRes() {
			...
			hideSourcechooserSelector() {
				...
				const selector = t.sourcechooserButton.querySelector(
						`.${ t.options.classPrefix }sourcechooser-selector`
					),
