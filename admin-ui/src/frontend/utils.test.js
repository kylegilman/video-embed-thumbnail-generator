...
describe( 'groupSourcesByResolution', () => {
	it( 'groups sources by resolution and sorts descending', () => {
		...
		expect( groups.map( ( g ) => g.res ) ).toEqual( [
			'1080',
			'720',
			'480',
		] );
...
describe( 'stripTimeParams', () => {
	it( 'removes an existing t param', () => {
		expect( stripTimeParams( 'https://example.com/?t=30' ) ).toBe(
			'https://example.com/'
		);
	} );

	it( 'removes an existing start param', () => {
		expect( stripTimeParams( 'https://example.com/?start=30' ) ).toBe(
			'https://example.com/'
		);
	} );

	it( 'preserves other query params', () => {
		expect( stripTimeParams( 'https://example.com/?foo=bar&t=30' ) ).toBe(
			'https://example.com/?foo=bar'
		);
	} );

	it( 'leaves a URL with no time params unchanged', () => {
		expect( stripTimeParams( 'https://example.com/?foo=bar' ) ).toBe(
			'https://example.com/?foo=bar'
		);
	} );
} );
...
describe( 'addStartTimeParam', () => {
	it( 'appends ?t=<seconds> when there is no existing query string', () => {
		expect( addStartTimeParam( 'https://example.com/', 30 ) ).toBe(
			'https://example.com/?t=30'
		);
	} );

	it( 'appends &t=<seconds> when a query string already exists', () => {
		expect( addStartTimeParam( 'https://example.com/?foo=bar', 30 ) ).toBe(
			'https://example.com/?foo=bar&t=30'
		);
	} );

	it( 'leaves the URL unchanged for zero or negative seconds', () => {
		expect( addStartTimeParam( 'https://example.com/', 0 ) ).toBe(
			'https://example.com/'
		);
		expect( addStartTimeParam( 'https://example.com/', -5 ) ).toBe(
			'https://example.com/'
		);
	} );
} );
