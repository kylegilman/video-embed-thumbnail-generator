...
class FFmpeg_Thumbnails {
	...
	public function filter_complex( $ffmpeg_watermark, $movie_height, $thumb = false ) {
		...
		if ( is_array( $ffmpeg_watermark )
			...
			$watermark_height = strval( round( intval( $movie_height ) * ( floatval( $ffmpeg_watermark['scale'] ) / 100 ) ) );
			...
			$overlay_watermark = '[scaled][watermark]overlay=' . $watermark_align . 'main_w*' . round( floatval( $ffmpeg_watermark['x'] ) / 100, 4 ) . ':' . $watermark_valign . 'main_h*' . round( floatval( $ffmpeg_watermark['y'] ) / 100, 4 );
	 ...
	 *
	 * This method creates a temporary file from the blob, then calls the main save() method
	 * to handle the file with the standardized naming convention.
	 *
	...
	public function save_from_blob( $attachment_id, $post_name, $file_info, $force_parent_id = 0, $force_featured = null ) {
		...
		$uploads  = wp_upload_dir();
		$temp_dir = trailingslashit( $uploads['path'] ) . 'thumb_tmp';
		wp_mkdir_p( $temp_dir );
		$temp_file_name = wp_unique_filename( $temp_dir, sanitize_file_name( $file_info['name'] ) );
		$temp_file_path = trailingslashit( $temp_dir ) . $temp_file_name;

		if ( ! move_uploaded_file( $file_info['tmp_name'], $temp_file_path ) ) {
			return array(
				'thumb_id' => false,
		...
		$temp_file_url = trailingslashit( $uploads['url'] ) . 'thumb_tmp/' . $temp_file_name;
	...
	public function save( $attachment_id, $post_name, $thumb_url, $thumbnail_index = false, $force_parent_id = 0, $force_featured = null ) {
		...
		if ( $existing_attachment_id ) {
			...
			$video_post_title = html_entity_decode( get_the_title( $attachment_id ), ENT_QUOTES, 'UTF-8' );
			$desc             = $video_post_title . ' ' . esc_html_x( 'thumbnail', 'text appended to newly created thumbnail titles', 'video-embed-thumbnail-generator' );
			if ( $thumbnail_index ) {
			...
			if ( $is_local_temp_file ) {
				...
				$final_poster_url = $uploads['url'] . '/' . basename( $final_posterpath );
