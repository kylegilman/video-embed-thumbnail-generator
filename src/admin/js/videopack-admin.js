		...
		jQuery.post(
			...
			function(data) {
				...
				if (data.lastthumbnumber == "break" || (kgthumbnailTimeout == null && i != 1) ) {
					i = parseInt( howmanythumbs ) + 1; } else {
					...
					if (parseInt( data.movie_width ) < parseInt( document.getElementsByName( maxwidthID )[0].value ) && jQuery( '#attachments-' + postID + '-kgflashmediaplayer-width' ).data( 'minimum' ) != 'on' ) {
						document.getElementById( widthID ).value = data.movie_width; } else {
				...
				function(){ // when the video is finished seeking
					...
					if ( thumbnail_saved.length > 0 ) { // if there are any thumbnails that haven't been generated
						...
						if ( thumbnail_saved.length > 0 ) {
							video.currentTime = thumbnail_saved[0]; } else {
...
function kgvid_enqueue_video_encode(postID, blogID) {
	...
	jQuery.post(
		ajaxurl,
		{ action:"kgvid_callffmpeg", security: kgflashmediaplayersecurity, movieurl: attachmentURL, ffmpeg_action: 'enqueue', encodeformats: kgvid_encode, attachmentID: postID, parent_id: parent_post_id, blog_id: blogID },
		function(data) {
			...
			jQuery.post(
				ajaxurl ,
				{ action:"kgvid_ajax_encode_videos", security:kgflashmediaplayersecurity } ,
				function(data) {
					jQuery( encodeprogressplaceholderid ).empty();
					jQuery( encodeplaceholderid ).append( data.embed_display );
...
function kgvid_set_singleurl() {
	...
	if ( url.length > 0 ) {
		jQuery.post(
			ajaxurl,
			{ action:"kgvid_sanitize_url", security: kgflashmediaplayersecurity, movieurl: url },
			function(data) {
...
function kgvid_insert_shortcode() {
	...
	if ( jQuery( '#attachments-' + basename + '-featured' ).is( ':checked' ) ) {
		var set_featured = "on"; } else {
		var set_featured = false; }

		jQuery.post(
			ajaxurl,
			{ action:'kgvid_callffmpeg', security: kgflashmediaplayersecurity, attachmentID: basename, movieurl: url, ffmpeg_action:'submit', poster: posterurl, parent_id: postid, set_featured: set_featured },
			function(data) {
...
function kgvid_rescan_external_server(movieurl, postID) {
	...
	jQuery.post(
		ajaxurl,
		{ action:"kgvid_rescan_external_server", security: kgflashmediaplayersecurity, movieurl:movieurl, postID:postID },
		function() {
...
function kgvid_delete_video(movieurl, postID, format, childID, blogID) {
	...
	if ( delete_for_sure == true ) {
		...
		jQuery.post(
			ajaxurl,
			{ action: "kgvid_delete_video", security: kgflashmediaplayersecurity, movieurl: movieurl, postid: postID, format: format, childid: childID, blogid: blogID },
			function(deleted) {
...
function kgvid_update_encode_queue() {
	...
	if ( kgflashmediaplayersecurity ) { // sometimes this tries to run after the media modal is closed
		...
		jQuery.post(
			ajaxurl,
			{ action:"kgvid_update_encode_queue", security: kgflashmediaplayersecurity, page: page },
			function(data) {
				...
				if ( data.queue.length !== 0 && data.queue.length !== undefined ) {
					jQuery.each(
						...
						function(video_key, video_entry) {
							...
							if ( page != "queue" && jQuery( '#attachments-' + queue_id + '-kgflashmediaplayer-encodeboxes' ).length == 0 ) {
								return true; }
...
function kgvid_redraw_encode_checkboxes(movieurl, postID, blogID) {
	...
	if ( kgflashmediaplayersecurity ) { // sometimes this tries to run after the media modal is closed
		...
		jQuery.post(
			ajaxurl,
			{ action:"kgvid_generate_encode_checkboxes", security: kgflashmediaplayersecurity, movieurl: movieurl, post_id: postID, page: page, blog_id: blog.ID, encodeformats: kgvid_encode },
			function(data) {
...
function kgvid_redraw_thumbnail_box(postID) {
	...
	if ( kgflashmediaplayersecurity ) {
		...
		jQuery.post(
			ajaxurl,
			{ action:"kgvid_redraw_thumbnail_box", security: kgflashmediaplayersecurity, post_id: postID },
			function(data) {
				if ( data.thumb_url ) {
					...
					jQuery( '#attachments-' + postID + '-thumbnailplaceholder' ).html( '<div class="kgvid_thumbnail_box kgvid_chosen_thumbnail_box"><span>' + data.thumb_error + '</span></div>' ); } else {
...
function kgvid_encode_queue(action, order, id, blogID) {
	...
	if ( action == "delete" ) {
		...
		jQuery.post(
			ajaxurl,
			{ action:"kgvid_clear_queue_entry", security: kgflashmediaplayersecurity, index: order, scope: blog.scope },
			function(data) {
				if ( data ) {
	...
	if ( action == "clear_completed" ) {
		...
		jQuery.post(
			ajaxurl,
			{ action:"kgvid_clear_completed_queue", security: kgflashmediaplayersecurity, type:"manual", scope: blog.scope },
			function(data) {
				if ( data ) {
					jQuery( 'table.widefat > tbody' ).replaceWith( "<tbody class='rows'>" + data + "</tbody>" );
				}
	...
	if ( action == "clear_queued" ) {
		...
		if ( clear_for_sure == true ) {
			...
			jQuery.post(
				ajaxurl,
				{ action:"kgvid_clear_completed_queue", security: kgflashmediaplayersecurity, type:"queued", scope: blog.scope },
				function(data) {
					if ( data ) {
						jQuery( 'table.widefat > tbody' ).replaceWith( "<tbody class='rows'>" + data + "</tbody>" );
					}
	...
	if ( action == "clear_all" ) {
		...
		if ( clear_for_sure == true ) {
			...
			jQuery.post(
				ajaxurl,
				{ action:"kgvid_clear_completed_queue", security: kgflashmediaplayersecurity, type:"all", scope: blog.scope },
				function(data) {
					if ( data ) {
						jQuery( 'table.widefat > tbody' ).replaceWith( "<tbody class='rows'>" + data + "</tbody>" );
					}
...
function kgvid_queue_control() {
	...
	jQuery.post(
		ajaxurl,
		{ action:"kgvid_queue_control", security: kgflashmediaplayersecurity, command:command, scope: blog.scope },
		function(success) {
			if ( success ) {
...
function kgvid_save_plugin_settings(input_obj) {
	...
	if ( jQuery( input_obj ).parents( ".kgvid_video_app_required" ).length ) {
		jQuery( input_obj ).parents( ".kgvid_video_app_required" ).append( save_span ); } else {
		...
		function kgvid_ajax_save() {
			...
			jQuery.post(
				ajaxurl,
				{ action:"kgvid_save_settings", security: kgflashmediaplayersecurity, setting: save_queue[0].id, value: setting_value, all_settings: all_settings },
				function(data) {
					...
					if ( data != false ) {
						...
						if ( save_queue[0].id == "width" || save_queue[0].id == "height" ) {
							...
							if ( save_queue[0].id == "height" ) {
								dimension = parseInt( data.validated_value ) + 25; } else {
						...
						if ( jQuery( '#app_path' ).data( 'ffmpeg_exists' ) == "on" && jQuery( input_obj ).hasClass( 'affects_ffmpeg' ) == true && jQuery( '#ffmpeg_output' ).length != 0 ) {
							...
							jQuery.post(
								ajaxurl,
								{ action: "kgvid_test_ffmpeg", security: kgflashmediaplayersecurity },
								function(data) {
						...
						if ( jQuery( '#ffmpeg_thumb_watermark_url' ).val() != '' && jQuery( '#app_path' ).data( 'ffmpeg_exists' ) == "on" && jQuery( input_obj ).hasClass( 'affects_ffmpeg_thumb_watermark' ) == true && jQuery( '#ffmpeg_output' ).length != 0 ) {
							...
							jQuery.post(
								ajaxurl,
								{ action: "kgvid_test_ffmpeg_thumb_watermark", security: kgflashmediaplayersecurity },
								function(thumb_url) {
									if ( thumb_url ) {
		...
		if ( save_queue == undefined || save_queue.length == 0 ) {
			var save_queue = [{ 'id':input_obj.id, 'value':setting_value }]; } else {
...
function kgvid_set_all_featured() {
	...
	if ( set_for_sure == true ) {
		...
		jQuery.post(
			ajaxurl,
			{ action: "kgvid_get_set_featured", security: kgflashmediaplayersecurity },
			function(count) {
...
function kgvid_switch_parents() {
	...
	if ( set_for_sure == true ) {
		...
		jQuery.post(
			ajaxurl,
			{ action: "kgvid_get_switch_parents", security: kgflashmediaplayersecurity, parent: new_parent },
			function(count) {
...
function kgvid_auto_generate_old(type) {
	...
	jQuery.post(
		ajaxurl,
		{ action: "kgvid_get_generating_old", type: type, security: kgflashmediaplayersecurity },
...
function kgvid_check_cms_progress(total, cms_type) {
	...
	if ( interval != "" ) {
		jQuery.post(
			ajaxurl,
			{ action: "kgvid_update_cms_progress", security: kgflashmediaplayersecurity, total: total, cms_type: cms_type },
			function(remaining) {
...
function kgvid_clear_transient_cache() {
	...
	jQuery.post(
		ajaxurl,
		{ action: "kgvid_clear_transient_cache", security: kgflashmediaplayersecurity },
		function() {
			...
			function() {
				...
				frame.on(
					...
					function() {
						...
						if ( $el.data( 'change' ).substr( -25 ) == "kgflashmediaplayer-poster" ) {
			...
			function() {
				...
				frame.on(
					...
					function() {
						...
						jQuery.post(
							ajaxurl,
							{ action:"kgvid_update_child_format", security: kgflashmediaplayersecurity, parent_id: parentID, video_id: video.id, format: format, blog_id: blog_id },
							function(data) {
...
function kgvid_clear_video(movieurl, postID, video_id, blog_id) {
	...
	jQuery.post(
		ajaxurl,
		{ action:"kgvid_clear_child_format", security: kgflashmediaplayersecurity, video_id: video_id, blog_id: blog_id },
		function(data) {
